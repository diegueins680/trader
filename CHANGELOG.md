...
- CLI/HTTP: fix `--idempotency-key` validation to enforce the documented `1..36` length limit, preserve negative millisecond epoch inputs in `--from/--to` numeric timestamp parsing, and harden exponential retry backoff math against integer overflow for very large retry counts.
