...
## Unreleased
- Dev/CI: make the local autoloop Codex backend stream `codex exec --json` events from stdin, forbid shell/web self-inspection during planner calls, scope patch-plan prompts to the selected files, default Codex reasoning effort to `low`, and enforce `AUTOLOOP_CODEX_TIMEOUT_MS`, so planner calls fail fast instead of hanging indefinitely while waiting for `--output-last-message` to flush.
