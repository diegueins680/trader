...
## Unreleased
- Formal methods/CLI: add `analyze-close-timing` to compute per-combo optimal close-time `tm` (max-PNL in `[ta, ta + 2*(tc-ta)]`) and robust `tm` distribution stats (`median`, `Q25/Q75`, `MAD`, lift), with an explicit integration rule for close decisions.
