...
## Unreleased
- Web UI: default `trader-config.js` now infers direct Fly API hosts only for split-app patterns (`*-web.fly.dev`, `*-web-hs.fly.dev`, or `*-web-hs-*.fly.dev`), avoids accidental rewrites like `*-webhook` or ambiguous `*-web-*` names, and fails over direct-host calls to `/api` on network errors and HTTP 404/502/503/504 (plus 401/403 for auto-inferred Fly direct hosts).
