...
- Dev/CI: harden Fly deploy workflow by retrying direct `flyctl` installation, downgrading repeated transient installer outages (for example `503 Service Unavailable`) to a warning + deploy skip, and still skipping deploy when Fly credentials/config are missing (`FLY_API_TOKEN` is required; the workflow uses `FLY_APP` when set, otherwise repo-root `fly.toml`).
