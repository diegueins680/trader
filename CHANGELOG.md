...
## Unreleased
- Dev/CI: make the local autoloop Codex backend stream `codex exec --json` events from stdin, forbid shell/web self-inspection during planner calls, scope patch-plan prompts to the selected files, default Codex reasoning effort to `low`, and split Codex timeouts between `AUTOLOOP_CODEX_TIMEOUT_MS` for idea/repair selection and `AUTOLOOP_CODEX_PATCH_TIMEOUT_MS` for `plan-patch`, so patch generation can run longer without slowing earlier planner steps.
