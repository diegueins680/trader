...
- Optimizer/Signals: add `optimize-equity` sampling for funding/OI-aware gating (`--p-funding-oi-aware`, `--funding-oi-funding-cap-min/max`, `--p-disable-funding-oi-funding-cap`, `--funding-oi-vol-lookback-min/max`, `--funding-oi-vol-cap-min/max`, `--p-disable-funding-oi-vol-cap`, `--funding-oi-size-mult-min/max`), so optimizer searches can tune funding/volatility-based entry blocking and size dampening instead of leaving that layer fixed.
