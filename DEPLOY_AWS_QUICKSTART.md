...
Notes:
- When `--distribution-id` is set, the script forces `apiBaseUrl` to `/api` in `trader-config.js` unless you pass `--ui-api-direct` (which keeps the full API URL and relies on CORS); `--api-url` is used to configure the CloudFront `/api/*` origin.
