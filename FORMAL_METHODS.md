...
`signalDirectionalityEntryAllowedForSide` in `haskell/app/Trader/SignalGates.hs`, together with the local `directionalityFreshEntryEfficiencyAllowed`, `directionalityWeakBandRegimeEvidenceOk`, and `directionalityWeakBandSignedZScoreOk` helpers, is the signed fail-closed admission check for fresh entries derived from `DirectionalitySnapshot`. `signalDirectionalityEntryAllowed` remains the side-agnostic convenience wrapper that admits only when at least one signed side is admissible.

Clauses:

1. `Nothing` is inadmissible, and every saved or live snapshot must first satisfy the efficiency-range invariant: `dsEfficiency` must be finite and in `[0,1]`. Non-finite, negative, or above-1 efficiency values are treated as malformed and cannot be reinterpreted as strong-band directional evidence.
...
5. For `0.40 < dsEfficiency <= 1`, the strong-band no-regime witness is preserved when no saved tuple is present at all, but any explicit malformed saved tuple still blocks as malformed.
6. Admissibility is monotone non-increasing as well-formed efficiency falls across the `0.40` review-band boundary and the `0.25` chop boundary, as saved HMM mass drifts farther outside the `1e-3` normalization tolerance under otherwise identical weak-band snapshots, and as the signed weak-band zScore for a fixed requested side moves toward and through zero.
7. During live weak-band review in `signalDirectionalitySnapshot`, `regime-bank-hysteresis` must itself be finite and non-negative before it can participate in the mean-reversion dominance comparison. Negative or non-finite hysteresis is treated as malformed and yields `NON_DIRECTIONAL_MALFORMED` rather than a permissive comparison result.
8. For fixed well-formed weak-band live regime data whose leader is `mr` with live gap `delta`, the MR-dominance veto is monotone in the configured hysteresis threshold: if `delta >= h` blocks at hysteresis `h`, then every smaller `h' <= h` also blocks. Equivalently, fresh-entry admissibility is monotone non-increasing as the configured hysteresis gap is lowered through the same live MR gap.
9. Mean-reversion-dominant weak-band snapshots remain blocked because `signalDirectionalitySnapshot` marks them `NON_DIRECTIONAL_MR`, and neither the signed helper nor the wrapper ever admits a snapshot with `dsNonDirectional = True`.
10. For fixed well-formed weak-band regime data, the long and short branches are exact mirror images under `zScore -> -zScore`; only the signed score for the requested side matters.
11. The side-agnostic wrapper is existential only: it may still admit a weak-band snapshot whose signed `zScore` supports one side, but it cannot admit missing snapshots or weak-band zero/non-finite scores because neither signed side is admissible there.

Bounded executable obligations:

- `testSignalGateDirectionalityWeakBandFailClosed` covers the strong-band no-change witness, explicit rejection of negative and above-1 efficiency on the saved-snapshot entry path, the `0.40` and `0.25` boundaries, side-specific signed `0.75` boundary acceptance for both long and short requests, opposite-sign rejection, zero rejection, malformed snapshot fail-closed behavior (including malformed efficiency and non-finite weak-band `zScore`), tuple sanity rejection, within-tolerance acceptance, preservation of the existing `NON_DIRECTIONAL_MR` veto, and the side-specific `Nothing` fail-closed case.
- `testSignalGateDirectionalityWeakBandMonotone` witnesses the non-increasing admissibility ladders as efficiency falls from `1.0` through the `0.40` review band and `0.25` chop cutoff when regime probabilities are missing, as the long-side signed weak-band `zScore` moves down through the `+0.75` floor and through zero, as the short-side weak-band `zScore` moves up through the `-0.75` floor and through zero, and as saved HMM mass drifts outside the normalization tolerance in the weak band.
- `testSignalGateDirectionalityWeakBandHysteresisFailClosed` covers an otherwise-admissible weak-band live snapshot whose MR leader gap stays below a valid hysteresis threshold, plus negative, `NaN`, and `Infinity` hysteresis rejection that now yields `NON_DIRECTIONAL_MALFORMED` and blocks both signed sides.
- `testSignalGateDirectionalityWeakBandHysteresisMonotone` witnesses the hysteresis ladder `[True, False, False, False]` as the configured weak-band MR gap threshold falls through the same live MR gap.

Proof sketch:

- `signalDirectionalityEntryAllowedForSide` now requires `directionalityFreshEntryEfficiencyAllowed`, and both `directionalityWeakBandRegimeEvidenceOk` and `directionalityWeakBandZScoreOk` short-circuit on `directionalityEfficiencyOk`, so non-finite or out-of-range efficiency cannot bypass weak-band review by falling through the `not (directionalityWeakBand eff)` branch.
- Because `directionalityFreshEntryEfficiencyAllowed eff` is `directionalityEfficiencyOk eff && eff > directionalityChopEfficiencyMax`, saved snapshots cannot reopen the documented chop veto by carrying a stale directional label, and malformed efficiency cannot be reinterpreted as strong-band directional evidence.
...
- `signalDirectionalityRegimeEvidence` now reuses the same tuple sanity check for live `RegimeProbs` and also requires `regime-bank-hysteresis` to be finite and non-negative before comparing the live MR gap, so negative, `NaN`, or `Infinity` hysteresis produces `mrDominant = Nothing`; weak-band snapshot construction interprets that as `NON_DIRECTIONAL_MALFORMED`, keeping malformed hysteresis fail-closed while leaving the strong-band no-tuple path unchanged.
- For fixed live MR gap `delta` and valid hysteresis, the veto predicate is exactly `delta >= hysteresis`, so lowering hysteresis can only preserve or enlarge the veto set; the admissible weak-band entry set is therefore monotone non-increasing as the configured hysteresis gap falls.
...
- The weak-band obligations are one-way: crossing from well-formed strong-band efficiency into `<= 0.40`, drifting from an in-tolerance HMM tuple to an out-of-tolerance tuple, moving the signed weak-band score for the requested side below `0.75`, or lowering the configured hysteresis below a fixed live MR gap can only remove admissible states; the long and short branches differ only by the sign map.
- Mean-reversion dominance still blocks via the existing `dsNonDirectional`/`dsReason` contract produced by `signalDirectionalitySnapshot`, so the repair only shrinks the admissible set for malformed-efficiency states, malformed-hysteresis weak-band states, and for weak-band states whose additive confirmation disagrees with the requested entry side while leaving the well-formed strong-band no-tuple path unchanged.
