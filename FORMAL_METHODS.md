...
- The IPv6 bracket normalization keeps bare/bracketed loopback authorities stable across `::1`, `::1/api`, `::1:PORT`, `[::1]/api`, and `[::1]:PORT`, and it bracketizes bare non-loopback IPv6 literals so `2001:db8::1/api` remains a direct host target.
- `portFromAuthority` only extracts IPv6 ports from bracketed authorities or the special `::1:PORT` loopback shorthand, so ambiguous bare non-loopback inputs such as `2001:db8::1:8443` preserve direct-host intent by treating the full authority as the literal, while bracketed `[2001:db8::1]:8443` still triggers the explicit non-`443` `http://` inference.
...
- `haskell/web/test/utils.test.mjs` mirrors this contract with regression rows for blank input, `/api` passthrough, bare relative same-origin rewrites, loopback inference (including bare/bracketed `::1` path preservation), non-loopback host inference (including `example.com`, bare/bracketed `2001:db8::1`, ambiguous bare `2001:db8::1:8443`, and bracketed `[2001:db8::1]:8443`), and explicit-URL preservation.
