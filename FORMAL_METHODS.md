...
5. Missing edges, negative fee floors, non-finite fee floors, or non-finite edges are fail-closed.
...
- `testTradingEntryGateFailClosedMonotone` and `testTradingEntryGateMalformedNoReopen` extend the `mkEntryGateState` witness so negative or non-finite per-side fees cannot reopen a blocked fresh entry.
