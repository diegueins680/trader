...
18. Every enabled signal-gate predicate that consumes edge, confidence, volatility, trend-price, forecast, current-price, threshold, or regime evidence must reject non-finite evidence (`NaN`, positive infinity, or negative infinity) before an admission comparison. Audit-only edge-spike mode may waive a finite spike-cap exceedance, but it must not waive the finite-evidence boundary. `signalDirectionalitySnapshotImplWithPredictionAndConfig` must reject a non-finite supplied predictor or current price before generic directionality evaluation, rather than validating them only when weak-band evaluation happens; consequently, a strong-trend path cannot bypass malformed prediction evidence. A rejected directionality witness returns `DirectionalitySnapshot True (Just "NON_DIRECTIONAL_MALFORMED")`; other rejected entry predicates return `False` or `(False, 0)` so the caller holds rather than opens exposure.
19. Before a config-aware entry predicate evaluates an admission comparison, every `Double` field of `SignalGateConfig` must be finite. This includes headroom, spike, trend, directionality, and predictor-tracking configuration fields. `NaN`, positive infinity, or negative infinity in any such field must produce a hold: config-aware Boolean entry gates return `False`, including audit-only and consecutive edge-spike paths, and directionality returns `DirectionalitySnapshot True (Just "NON_DIRECTIONAL_MALFORMED")`.
...
- `testSignalGateNonFiniteEvidenceFailsClosed` covers `NaN`, positive infinity, and negative infinity for audit-only and consecutive edge-spike authorization, meta-label edge/confidence thresholds, funding and OI-volatility risk thresholds, trend-price confirmation, prediction-aware forecast/current-price weak-band and full-snapshot directionality, and regime hysteresis/probability directionality evidence; it also preserves representative finite admissible boundaries.
- `testSignalGateNonFiniteConfigFailsClosed` covers `NaN`, positive infinity, and negative infinity in every `Double` field of `SignalGateConfig`, including the audit-only and consecutive spike paths, headroom, fee-buffer, trend, weak-band, and directionality admission boundaries; representative finite defaults remain admissible.
...
- The non-finite signal-gate invariant follows by case analysis over all entry-authorizing numeric predicates. Audit-only and consecutive spike paths first require a finite normalized threshold and finite edge; `signalMetaLabelOk` requires finite thresholds and supplied edge/confidence; `signalFundingOiCheck` rejects non-finite funding pressure, OI volatility, caps, and sizing evidence when enabled; trend confirmation rejects non-finite price or SMA evidence; prediction sanity rejects non-finite current price or prediction; and `signalDirectionalitySnapshotImplWithPredictionAndConfig` applies `decisionEvidenceFinite` to current price and any supplied forecast before it delegates to generic directionality evaluation. The weak-band helper independently checks z-score, current price, and supplied forecast, so neither weak nor strong directionality paths can admit malformed prediction evidence. `signalRegimeEdgeOk` likewise rejects a non-finite regime-edge threshold before normalization. Each failing case therefore reaches a rejecting Boolean/result or the malformed non-directional snapshot, and no comparison with a non-finite value can admit an entry.

- `signalGateConfigFinite` is evaluated before every config-aware admission comparison. It checks all `Double` fields of `SignalGateConfig`, so no `NaN` comparison can silently become false and no infinity can widen a cap or threshold. The same guard is explicit on audit-only and consecutive edge-spike admission paths, preventing their exemptions from reopening an entry under malformed configuration. The directionality implementation maps an invalid configuration to the malformed non-directional snapshot before window or weak-band thresholds are evaluated.
