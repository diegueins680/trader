...
9. The side-agnostic wrapper is existential only: it may still admit a weak-band snapshot whose signed `zScore` supports one side, but it cannot admit missing snapshots or weak-band zero/non-finite scores because neither signed side is admissible there.
