...
18. Every enabled signal-gate predicate that consumes edge, confidence, volatility, trend-price, prediction-price, or regime evidence must reject non-finite evidence (`NaN`, positive infinity, or negative infinity). Audit-only edge-spike mode may waive a finite spike-cap exceedance, but it must not waive the finite-evidence boundary. A rejected directionality witness returns `DirectionalitySnapshot True (Just "NON_DIRECTIONAL_MALFORMED")`; other rejected entry predicates return `False` or `(False, 0)` so the caller holds rather than opens exposure.
...
- `testSignalGateNonFiniteEvidenceFailsClosed` covers `NaN`, positive infinity, and negative infinity for audit-only and consecutive edge-spike authorization, meta-label edge/confidence checks, funding and OI-volatility checks, trend-price confirmation, and regime hysteresis/probability directionality evidence; it also preserves representative finite admissible boundaries.
...
- The non-finite signal-gate invariant follows by case analysis over all entry-authorizing numeric predicates. Audit-only and consecutive spike paths first require a finite normalized threshold and finite edge; `signalMetaLabelOk` requires finite supplied edge and confidence; `signalFundingOiCheck` rejects non-finite funding pressure, OI volatility, caps, and sizing evidence when enabled; trend confirmation rejects non-finite price or SMA evidence; prediction sanity rejects non-finite current price or prediction; and directionality rejects non-finite hysteresis, supplied regime probabilities, efficiency, or z-score evidence before a directional snapshot is emitted. Each failing case therefore reaches a rejecting Boolean/result or the malformed non-directional snapshot, and no comparison with a non-finite value can admit an entry.
