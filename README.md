...
- When a local bounded cycle pushes directly to the default branch, it waits for the GitHub Actions `CI` workflow by querying Actions runs for the exact pushed SHA and tolerates delayed workflow registration longer before treating the push as failed; if no CI run appears, the status/error includes the commit's visible check suites for diagnosis.
