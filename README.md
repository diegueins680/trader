...
The research-only market-context artifact consumes an abstract scoped factor value directly. That value retains the validated target, quote, selected peer identities, bar, interval, and feature row; its private constructor prevents callers from relabeling a generic row as evidence from another scope. Causal universe witnesses may arrive after the peer bar end but no later than the factor decision. Training returns are separate abstract values computed from canonical complete target bars whose asset, interval, start event, and horizon are bound before fitting, so callers cannot attach a generic return from another asset. Before OLS, the exact ordered factors, feature values and availability witnesses, and targets must match the request's versioned canonical observation digest; the upstream data, source-manifest, and split hashes remain separately bound. Inference requires the event time to lie on the exact registered validation grid and inside its inclusive start/end range. The complete forward horizon of every accepted validation event must also end strictly before the separately bound final-holdout start.
...
Each cached observation records the underlying event time separately from the time it became available. Sources with provider release or vintage evidence must name it with `availableTimeField`; if a changed payload reuses the same explicit availability timestamp and revision, collection fails closed and preserves the original row until the provider supplies distinct release timing or revision identity. Sources without an explicit availability field become available only when the collector first sees them, and a later changed value is recorded as a new first-seen release instead of being backdated to the historical event. Unchanged re-fetches preserve the original first-seen timestamp. The panel builder uses only availability timestamps, applies publication lags and maximum ages, causally normalizes each metric before combining metrics, and emits coverage alongside every family. Stateful readings are carried forward only until their configured age limit; event/flow readings reset between bars.
...
Empty `mean` buckets remain missing through normalization and emit a neutral family value instead of manufacturing a zero-valued observation.
...
`run` combines collection and panel construction. A provider failure is isolated and reported as degraded; successful sources are still cached atomically, while the command returns non-zero unless `--allow-partial` is explicit. A panel manifest records the ordered `external_feature_panel_v2` schema, `feature_availability_v2` coverage semantics, and SHA-256 hashes of the exact cache, bar grid, and panel. `verify-panel` checks those hashes; recomputes the eligible observation and metric populations; rejects incompatible columns, unordered timestamps, scope drift, non-finite values, out-of-range coverage, or a non-zero feature with zero coverage; and deterministically reconstructs the panel for byte comparison. Relocated artifacts may be supplied explicitly with `--cache`, `--bars`, and `--panel`; their bytes must still match the recorded hashes. After that verification, a pure opt-in Haskell decoder can enforce the same 40-column order and retain each family value plus fractional coverage, so an observed zero is distinct from unavailable zero. The decoder does not itself verify manifest hashes and is not connected to existing feature construction or trading orchestration. Because this panel version does not retain each selected source event/availability timestamp, it cannot feed the timestamp-preserving model adapter without a later explicit panel schema. Generated family headers—including `microstructure`, `options_vol`, and `onchain`—remain accepted directly by the legacy Haskell CSV loader. Secrets referenced by `queryFromEnv` or `headersFromEnv` are read only from the environment and are never stored or printed.
...
In source configuration, `entity`/`entityField` identifies the target trading asset, not an internal provider record ID; an empty entity is global. Panel construction requires either `--symbol` (which includes that full/base asset plus global observations) or an explicit `--global`; generated symbol rows apply only to that full or base asset. When the Haskell backtest target symbol is unresolved, symbol-scoped rows are ignored and only global observations are admitted. Blank, malformed, `NaN`, and infinite cells are rejected fail closed, and a feature family is neutral-filled only after at least one finite causally available observation overlaps the requested bar grid, so future-only or cross-asset rows cannot synthesize all-zero exogenous features. Enabling alternative data changes the predictor feature dimension, so models must be retrained and validated through walk-forward/holdout tests. Collection alone does not authorize a signal for live trading, and missing data stays neutral rather than being interpreted as a bullish or bearish observation. Some fitted Haskell feature rows currently represent an unavailable optional channel with the same numeric zero as an observed zero. The additive `feature_availability_v2` contract preserves event time, availability time, ordered field identity, required/optional status, and an explicit observed mask; its external-family bundle also retains the selected timestamps and masks through revision-safe alignment, while the old bundle remains an unchanged dense compatibility projection. The opt-in `external_family_model_features_v2` adapter maps that timestamp-preserving bundle into the legacy 38-value level/delta order plus 38 availability masks; the opt-in `binance_derivatives_model_features_v2` adapter similarly maps verified, contiguous derivatives rows into the legacy five-value order plus five masks. The separate opt-in `coinbase_cross_exchange_model_features_v2` contract reproduces the legacy five basis, z-score, and return-spread formulas on complete inputs while retaining five masks and causal witnesses; it requires exact same-bar evidence and never forward-fills missing Coinbase buckets. The opt-in `complete_ohlcv_feature_inputs_v2` boundary takes the stricter complete-case policy for core market bars: all OHLCV fields and first-seen timing are required, and only a valid exact-grid bundle may project into the unchanged legacy price formulas. Its projection strips unrelated optional channels. The separate `point_in_time_liquidity_universe_v2` contract selects only from one coherent quote-scoped snapshot that was first seen by the decision, is within its event-age limit, and carries explicit contemporaneous eligibility; revisions become usable only from their own availability time, ties are deterministic, and raw volume weights normalize without overflow. The dedicated `binance_usdm_market_context_panel_v2` decoder binds a prospective derived population and its optional exact-bar peer cells to a frozen source-manifest digest, exact population count and ordinals, rolling-volume window timestamps, and first-seen clocks, then projects only through that canonical selector. Digest syntax is not proof of raw response bytes or completeness, so the referenced manifest must be independently verified before research admission. The isolated `point_in_time_market_context_factor_v2` adapter consumes one such selection per bar, excludes the target, renormalizes the requested ranked peer weights, and exposes the raw volume-weighted peer return with a causal timestamp and explicit availability mask. It matches the peer-basket portion of the legacy raw weighted-return formula on complete evidence, without its optional same-asset Coinbase augmentation. The separate research-only `point_in_time_market_context_linear_ols_v1` component fits that factor only on an exact causal training grid with horizon-specific realized labels and a declared purge/embargo gap. It records unavailable rows without fitting their dense zeros, binds fold/source/code/runtime/cost provenance, hashes a strict versioned payload, and returns no estimate for unavailable or incompatible evidence; its output is a non-actionable mean/variance summary. It is not a complete candidate artifact, data-admission receipt, experiment runner, holdout gate, or deployment format. The legacy fitted intercept, beta, residual, and derived context features remain unchanged rather than silently acquiring these semantics. All v2 and research-artifact boundaries remain unimported by legacy features, predictors, bots, or execution paths. Current historical OHLCV and Coinbase candle paths do not prove original per-bar first-seen/publication times, and the legacy universe CSV has no separate availability/completeness witness, so they cannot be projected into these boundaries by inventing those facts. Remaining source adapters, feature builders, full candidate artifacts, and production builders stay on their legacy or incomplete schema, so `FEATURE-MISSINGNESS-001` remains open and blocks promotion of an affected learned candidate until every required source policy and explicit migration is validated.

Prospective USDⓈ-M market-context evidence has a separate offline trust boundary. `python3 scripts/research/market_context_source.py verify --manifest <frozen-bundle>/source-manifest.json --panel-output <output>/market-context.csv --receipt-output <output>/verification-receipt.json` hashes and strictly decodes every registered public response, reconstructs the contemporaneous eligible population and exact close-to-close peer returns, and emits the deterministic `binance_usdm_market_context_panel_v2` panel plus a non-authorizing receipt. The command accepts no network or credential input. It is not a collector, and success does not authorize research use, model fitting, holdout access, orders, promotion, deployment, or live trading.

For a collector-produced bundle, use the stronger wrapper: `python3 scripts/research/verify_market_context_bundle.py verify --status <frozen-bundle>/collection-status.json --panel-output <output>/market-context.csv --receipt-output <output>/bundle-receipt.json`. It accepts only the exact `complete_unverified` success status with a published manifest and zero downstream authority, binds the status and manifest bytes to the historical collector, source-verifier, and source-license blobs at the recorded Git commit, requires the executing bundle and source verifiers to be those exact committed versions, then delegates raw-response reconstruction to the source verifier. `collecting`, `partial_failure`, `cleanup_pending`, `cleanup_failure`, version drift, malformed, changed, or authorizing evidence fails closed. Its deterministic receipt is integrity evidence only and explicitly grants no research admission, experiment, holdout, model, promotion, deployment, order, or live authority.

Replay a saved bundle receipt against the exact frozen external archive with `python3 scripts/research/verify_market_context_receipt.py --receipt <saved-bundle-receipt.json> --archive <frozen-bundle>`. Replay is offline and read-only. It requires the executing bundle-verifier bytes to equal the version stored at the collection commit, permits exactly the status, manifest, and registered raw files in the archive, hashes the complete inventory before and after verification, reconstructs the panel, and compares the full receipt with type-sensitive JSON semantics. A valid replay is still not research admission and carries the same explicit zero-authority boundary.

The separate prospective collector is `python3 scripts/research/collect_market_context.py collect --output-dir <new-bundle-dir> --interval <1h|4h|8h> --bar-open-time <aligned-epoch-ms>`. It is restricted to fixed public Binance GET endpoints, refuses redirects and existing output directories, binds its loaded collector/verifier/source-license bytes to one captured Git commit and revalidates that exact binding before publication, applies bounded response sizes, an end-to-end transport deadline, per-eligible-member ticker freshness, and conservative request-weight limits, durably flushes and rechecks the causal window at atomic publication, and publishes only a `complete_unverified` source bundle. Recovery durably replaces any possibly visible success status with an indeterminate `cleanup_pending` sentinel before removing the manifest; only confirmed removal may advance to `partial_failure`, while a failed removal advances to `cleanup_failure` when that status can be written. An interrupted or unconfirmed cleanup remains explicitly indeterminate and requires manual review. Run the offline verifier separately. Do not start the registered missingness-aware candidate's collection before 2027-01-21, and do not treat collection or verification as experiment, holdout, model, order, deployment, or live authority.

## Predictor identity and 2026 evidence review
...
The job runs at minute `10` of each hour, does not load exchange credentials, never uses `KeepAlive`, and records bounded per-symbol status under `data/research/.collector/last-run.json` by default. `TRADER_RESEARCH_CACHE` may point to a persistent absolute path, `TRADER_RESEARCH_SYMBOLS` overrides the fixed universe, `TRADER_RESEARCH_COLLECT_MINUTE` changes the minute after the hour, and `TRADER_RESEARCH_MAX_RUN_SECONDS` sets the wall-clock deadline (default `3000`; the installer accepts `60` through `3500`). The atomic plist is the installed configuration source, so later `status`, `restart`, and `uninstall` commands do not require those overrides or another Python executable on `PATH`. The canonical symbol list remains stable, while actual requests rotate deterministically by UTC epoch hour under `utc_epoch_hour_rotation_v1`; status records the exact `requestOrder`, and verification rejects a non-permutation or an order inconsistent with `startedAt`. Legacy schema-3 statuses without either additive request-order field retain their historical fixed-order meaning.

Every cache-mutating entry point shares one writer lock, and panel reads hold it across the complete cross-symbol snapshot; scheduled overlap exits cleanly while direct and campaign work waits instead of racing a read/merge/replace transaction. CSV, status, and plist files use same-filesystem atomic replacement, so interruption cannot expose a partial file. A symbol is `ok` only when the refresh fetched a current closed kline and every derivatives series returned a finite observation inside its fixed family freshness bound. Empty or stale klines and empty, errored, or out-of-bound funding, OI, basis, or taker evidence make the run a nonzero `partial_failure` without starving later symbols. The public client uses conservative process-local request-weight, funding, and futures-stat budgets and reconciles Binance's `X-MBX-USED-WEIGHT-1M` response header. Ordinary source failures remain isolated, but HTTP 418/429 or JSON `-1003` opens a run-level circuit: the affected symbol records sanitized throttle metadata, every later symbol is explicitly skipped without another request, and the run fails. Binance can publish a completed period after its corresponding kline close: an in-bound trailing bucket may remain unavailable, is counted as `trailingUnavailable`, and never becomes an observed value. New point-in-time alignment stops carrying an observation past its freshness bound, while merge precedence retains newer non-null evidence already accumulated from an earlier successful response.

Because those Binance history endpoints do not expose an independent publication or revision timestamp, the collector also keeps `binance_derivatives_first_seen_v2` raw ledgers under `data/research/.observations/`. A fetched row is unavailable before the fetch completed; unchanged refetches preserve that first-seen time, changed values become later revisions, and existing legacy cache cells are never assigned inferred v2 provenance. The bar CSV adds `V2Value`, `V2Observed`, `V2Fresh`, `V2EventTime`, and `V2AvailabilityTime` columns for each of `funding`, `oi`, `basis`, and `taker`; overlap merges preserve that canonical family/field order instead of accepting pandas' alphabetical reordering. Stale or unavailable v2 values are dense zero only with an explicit false freshness mask, while observed zeros keep true observed/fresh masks. Collector status schema 3 fails green status when a fresh v2 tail or raw ledger is malformed, the code commit is unavailable, or the collector, datafeed, shared limiter source, and source-license manifest differ from that commit; it records Python/numpy/pandas versions and binds every accepted bar/ledger file by absolute path, row count, and SHA-256 under `binance_derivatives_collection_artifacts_v3`. A read-only verifier requires a complete-pass status and valid code provenance, then independently checks artifact paths and hashes, raw CSV headers, scopes, rows, grids, v2 coverage, fresh-tail versioning, and causal reconstruction from each ledger. Rehashing a changed bar file therefore cannot make inconsistent v2 cells admissible. A pure opt-in Haskell decoder independently enforces the row contract; it does not itself verify the manifest or file hashes and remains disconnected from production feature construction, artifacts, predictors, and trading.

Verify the most recent complete collection before copying its status SHA-256 into any frozen experiment receipt:
...
`--cache-dir` may point to a relocated copy only when every artifact byte still matches the recorded digest. Schema-2 status files remain historical operational evidence but are not artifact manifests; the next collection rewrites `last-run.json` as schema 3. The status file is mutable across scheduled runs, so a decisive experiment must freeze it or record the verifier-returned status digest before fitting.
...
The checked-in Hetzner trading profile is the sole live executor. Its current bounded relaxation pins five reviewed UUIDs for AVAX, UNI, SUI, ETC, and ADA, starts at most two workers per polling cycle, disables their online combo replacement, and initially runs independent `shadow` selection. The runtime rechecks each allowlisted row and refuses raw, quarantined, or unknown processing tiers even if an earlier annotation or operator list is stale. Each adopted combo is capped at 1%, and any larger inherited minimum-size floor is reduced to that cap, so the five-worker configuration can enter but cannot exceed 5% aggregate configured exposure; existing-position recovery still takes priority and bypasses ordinary fleet caps. Exchange-native protection-order management is enabled for strategy-defined stop-loss/take-profit exits. Starting with the first full UTC day after `2026-08-27T05:48:21Z`, automatic graduation requires a boundary-fresh equity baseline for every reviewed UUID, 10 complete days present for every reviewed UUID, positive aggregate window-relative fleet return, maximum drawdown at or below 10%, at least 10 order attempts with 95% successful submission reliability, at least 99% healthy running status intervals, and an actual latest status that is live, trade-enabled, running, unhalted, and error-free for every UUID. Each worker is rebased to an equity observation no more than 15 minutes before the boundary, so stale state and gains or losses before the review window cannot affect graduation. Expected 15-minute status intervals for every worker form the reliability denominator; a missing interval is charged as unhealthy instead of disappearing from the evidence. It then also requires the strict 10-day portfolio selector to produce a valid enforcement selection. The review runs hourly, persists pending evidence beside the leaderboard, and writes a durable `portfolio.graduated` decision before effective mode changes to `enforce`; database, evidence, selector, or persistence failure leaves shadow mode and the UUID target allowlist active. The 1%/5% exposure caps and normal risk controls are not removed by graduation. The override does not change optimizer discovery thresholds or promote any unnamed candidate. Fly omits `--binance-live` and sets `TRADER_BOT_TRADE=false`, making it a read-only standby even when CI recreates its app machine. Research profiles request 1,100-day lookbacks on 6h-or-longer intervals and reserve 64 of the bars actually fetched before deriving a safe capped window; exchange pagination and shorter symbol histories therefore cannot land just below the train/tune/backtest split requirement while enough OOS coverage remains for the default 180-day evidence floor. Long-horizon searches run as recurring batches of four broad primary trials and, when needed, six activity-focused recovery trials, so the two passes explore distinct neighborhoods and completed evidence is merged incrementally. The recovery edge band remains cost-safe at `0.0018..0.0024`. The managed Hetzner research profile optimizes Sharpe and forwards that objective to both outer candidate scoring and nested threshold tuning. Exposure metrics use the position held during each return interval, while the position series remains the end-of-bar state; same-bar exits therefore retain their real exposure without appearing open after the bar. A simulated stop-loss, trailing-stop, or full take-profit filled at index `t+1` also forces that event to remain flat, so the earliest fresh re-entry is `t+2` even when configured cooldown is zero. Hetzner research caps each optimizer child at an 8 GiB Haskell heap with `TRADER_OPTIMIZER_TRIAL_HEAP_CAP=8g`, preserving headroom for the API and PostgreSQL while recording heap-overflow trials as failures. Hetzner research favors faster TA methods and refreshes only 20 stale incumbents daily after seven days, leaving most CPU for discovery. Optimizer telemetry distinguishes `optimizer.auto.admitted` from `optimizer.auto.no_admission`, includes board counts and newest timestamps, and reports database persistence failures instead of treating every successful merge process as a new combo.

Database fallback rows without `processing.tier` are treated as unknown and cannot be activated by `TRADER_TOP_COMBO_DEPLOYABLE_OVERRIDE_UUIDS`. Valid UUID values are canonicalized before lookup and deduplication, so equivalent uppercase and lowercase spellings select the same reviewed candidate.

Automatic graduation also requires one current healthy status for every reviewed worker. `TRADER_PORTFOLIO_AUTO_GRADUATE_MAX_BASELINE_AGE_SEC`, `TRADER_PORTFOLIO_AUTO_GRADUATE_STATUS_INTERVAL_SEC`, and `TRADER_PORTFOLIO_AUTO_GRADUATE_MAX_LATEST_STATUS_AGE_SEC` each default to 900 seconds. They bound the pre-window baseline, define the expected per-worker status-coverage slots, and bound current liveness respectively; missing, stale, future-dated, or unhealthy evidence leaves shadow targeting and the UUID safeguards active.
...
TRADER_PORTFOLIO_SELECTOR_MIN_DAYS=10
...
TRADER_PORTFOLIO_AUTO_GRADUATE_STARTED_AT_MS=1787809701000
TRADER_PORTFOLIO_AUTO_GRADUATE_MIN_DAILY_OBSERVATIONS=10
...
TRADER_PORTFOLIO_AUTO_GRADUATE_MAX_BASELINE_AGE_SEC=900
TRADER_PORTFOLIO_AUTO_GRADUATE_STATUS_INTERVAL_SEC=900
...
Before replacing Caddy, the deploy preserves the running container's effective configuration. A later failure restores and recreates both the prior API image and prior proxy configuration before validating rollback health.
