...
- Live kill switches are now optimizable via `--p-disable-max-dd`, `--max-dd-min/max`, `--p-disable-max-dl`, `--max-dl-min/max`, `--p-disable-max-wl`, `--max-wl-min/max`, and `--p-disable-max-oe` / `--max-oe-min/max`, so optimizer runs can search drawdown, daily-loss, weekly-loss, and order-error halts instead of leaving them fixed.
