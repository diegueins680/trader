...
Short OPE validates its complete requested window, integer controls, episode budget
and aligned symbol/series coverage before sampling. Admission checks representation
and bounds without scanning future values. See the [OPE window-admission audit](research-notes/sequential-control-2026-09-17/ope-window-admission-audit.md).

Economic reports require stopped replays. Successful completion also requires the
...
All compact reports are prepared before the output directory is created and written as UTF-8 bytes with LF terminators; invalid memory metadata, malformed report fields and serialization failures leave no partial report. Filesystem write errors still require operator cleanup.
