...
Because those Binance history endpoints do not expose an independent publication or revision timestamp, the collector also keeps `binance_derivatives_first_seen_v2` raw ledgers under `data/research/.observations/`. A fetched row is unavailable before the fetch completed; unchanged refetches preserve that first-seen time, changed values become later revisions, and existing legacy cache cells are never assigned inferred v2 provenance. The bar CSV adds `V2Value`, `V2Observed`, `V2Fresh`, `V2EventTime`, and `V2AvailabilityTime` columns for each of `funding`, `oi`, `basis`, and `taker`; overlap merges preserve that canonical family/field order instead of accepting pandas' alphabetical reordering. Stale or unavailable v2 values are dense zero only with an explicit false freshness mask, while observed zeros keep true observed/fresh masks. Collector status schema 2 fails green status when a fresh v2 tail or raw ledger is malformed. A pure opt-in Haskell decoder independently enforces complete ordered field groups, finite/binary values, causal timestamp witnesses, freshness limits, and a non-actionable zero for stale or unavailable cells. It does not verify a manifest or file hash and remains disconnected from production feature construction, artifacts, predictors, and trading.
