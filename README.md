...
For a collector-produced bundle, use the stronger wrapper: `python3 scripts/research/verify_market_context_bundle.py verify --status <frozen-bundle>/collection-status.json --panel-output <output>/market-context.csv --receipt-output <output>/bundle-receipt.json`. It accepts only the exact `complete_unverified` success status with a published manifest and zero downstream authority, binds the status and manifest bytes to the historical collector, source-verifier, and source-license blobs at the recorded Git commit, then delegates raw-response reconstruction to the source verifier. `collecting`, `partial_failure`, `cleanup_pending`, `cleanup_failure`, malformed, changed, or authorizing evidence fails closed. Its deterministic receipt is integrity evidence only and explicitly grants no research admission, experiment, holdout, model, promotion, deployment, order, or live authority.
