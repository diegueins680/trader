...
- If the forever runner hits a general dirty-worktree block outside a recoverable failed cycle, it now checkpoints that work onto an `autoloop/dirty/<base-branch>/...` branch, pushes the checkpoint commit with a short path analysis, returns to the loop branch, and only blocks if that checkpoint recovery fails.
