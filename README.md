...
The checked-in Hetzner trading profile is the sole live executor. It reserves two startup slots for `ARBUSDT,DOTUSDT`, starts both in one recovery cycle, disables automatic combo substitution, and forces `ta_best` so a model-training stall cannot strand exchange inventory during a redeploy. Fly omits `--binance-live` and sets `TRADER_BOT_TRADE=false`, making it a read-only standby even when CI recreates its app machine. Research profiles use 1,100-day lookbacks on 6h-or-longer intervals: their 20% OOS curves contain about 220 daily observations while remaining within the 5,000-bar process cap, so candidates can satisfy the 180-day evidence floor. Long-horizon searches run as recurring batches of four primary trials and, when needed, six relaxed-discovery trials so completed evidence is merged incrementally rather than withheld behind an hours-long batch. The production discovery edge band remains `0.0018..0.0024`; optimized combos can still be evaluated in research and shadow roles without overriding the live recovery preset.
