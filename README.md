...
- Each bounded autoloop cycle now records explicit phases for selecting one valuable change, reviewing a local `haskell/web/src/*` UI file for safe UX fixes, reviewing a correctness/formal artifact (`FORMAL_METHODS.md` or test files) with an explicit invariant/property, then verifying, commit/push, and waiting for GitHub CI.
