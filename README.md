...
- If that red run is only a `fourmolu --mode check` diff on editable Haskell files, the bounded cycle now self-heals it deterministically by formatting those exact files with `fourmolu -i`, rerunning the formatter check plus normal Haskell verification, and pushing the repair without waiting on the model to rediscover a formatting-only fix.
