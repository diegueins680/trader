...
- Optional repo variable / dispatch input: `AUTOLOOP_MODEL` or `model` to override the default model slug; `AUTOLOOP_MAX_ITERATIONS` or `max_iterations` bounds same-run repair attempts; `dry_run=true` skips commit/push/merge.
...
- Autoloop guardrails are intentionally narrow: it cannot edit workflows, deploy files, lockfiles, package manifests, or secrets; verification commands are taken from a fixed allowlist; and commits stage only the planned changed files so generated byproducts do not leak into bot PRs.
...
- Before reusing or force-updating `autoloop/<base-branch>`, the bounded runner refreshes the remote loop branch head and uses an explicit `refs/heads/...` lease, which avoids stale `--force-with-lease` rejections after previous cycles have already updated the branch.
- When a local bounded cycle pushes a PR update, it now waits for the GitHub Actions `CI` workflow by querying Actions runs for the exact pushed SHA and tolerates delayed workflow registration longer before treating the PR as failed; if no CI run appears, the status/error includes the commit's visible check suites for diagnosis.
