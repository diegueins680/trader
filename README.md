...
- When the local planner uses Codex CLI, it now sends prompts over stdin and consumes `codex exec --json` events instead of waiting on `--output-last-message`; `AUTOLOOP_CODEX_TIMEOUT_MS` bounds each Codex planner call (default `300000`) so stalled planner sessions fail fast instead of hanging the cycle in `choose-change` or `plan-patch`.
