...
Direct `apiBaseUrl` requests can also fail over to `apiFallbackUrl` on 401/403/404/502/503/504 responses (plus network errors), so keep auth/cors settings consistent between primary and fallback hosts.
...
- Web UI: default `trader-config.js` now auto-infers a direct API URL for Fly split apps named like `*-web.fly.dev` or `*-web-hs.fly.dev` (for example, `trader-web-hs.fly.dev` -> `https://trader-hs.fly.dev`) and sets `/api` as fallback. Names that only contain `-web` as a substring (for example `price-webhook.fly.dev`) are not rewritten, and `-web-` inference is intentionally limited to `...-web-hs...` suffixes to avoid ambiguous rewrites. Override `apiBaseUrl`/`apiFallbackUrl` explicitly if your host naming differs.
