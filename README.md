...
- When the local planner uses Codex CLI, it now sends prompts over stdin, consumes `codex exec --json` events, explicitly forbids shell/web self-inspection, and keeps patch-plan prompts scoped to the selected files instead of the full repo context; `AUTOLOOP_CODEX_TIMEOUT_MS` bounds each Codex planner call (default `300000`) so stalled planner sessions fail fast instead of hanging the cycle in `choose-change` or `plan-patch`.
