...
- The persisted `metrics.closeTiming.recommendedMaxHoldBars` recommendation mirrors the analyzer's positive-lift `Q75` optimal duration; the later backfill rerun is the step that decides whether that retune is actually valid enough to persist into `params.maxHoldBars`.
