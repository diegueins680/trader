...
Every live Binance futures entry now passes a final, fail-closed market-risk boundary. The boundary walks the live order book for the actual requested quantity, enforces spread and expected-impact ceilings, charges side-adverse funding against the signal's edge budget, rejects extreme mark/index basis, and blocks high, stale, or unavailable symbol-level ADL evidence. It never blocks position reductions or protective orders. Current open interest, taker buy/sell ratio, and historical basis are recorded in the order evidence but remain shadow-only until out-of-sample validation justifies using them as alpha. `TRADER_MARKET_RISK_*` configures freshness and cost ceilings; setting `TRADER_MARKET_RISK_FAIL_CLOSED=false` is an explicit unsafe availability tradeoff. Binance klines also retain quote volume, trade count, and taker-buy base/quote volume instead of discarding those exchange fields.

```dotenv
TRADER_PORTFOLIO_SELECTOR_ROLLOUT_MODE=canary
