...
The UI can auto-apply top combos (on by default) and auto-disables after you edit combo-driven fields; it shows when a combo auto-applied and can auto-start missing bots for the top 10 unique combo symbols (Binance only) once interval/lookback validation passes. Manual override locks include an unlock button to let combos update those fields again.
...
On Fly split-app hosts (`*-web-hs.fly.dev` style), the listenKey stream now auto-prefers an inferred direct API host (`https://*-hs.fly.dev`) when `apiBaseUrl` is `/api` and no explicit `apiFallbackUrl` is set, reducing SSE disconnects like `ERR_HTTP2_PING_FAILED` through proxies.
