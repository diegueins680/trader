...
Notes:
- The UI config defaults `apiBaseUrl` to the direct API URL even when a distribution ID is set. Use `--ui-api-proxy` (or `TRADER_UI_API_MODE=proxy`) to force `/api` when the `/api/*` behavior is configured.
