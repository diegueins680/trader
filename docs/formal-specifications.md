...
The isolated `missingness_aware_calibrated_shallow_feature_panel_v1` composes only registered future, pre-holdout `complete_ohlcv_feature_inputs_v2` rows with optional exact-scope derivatives and Coinbase rows. It freezes twelve required 6/24-bar price fields followed by ten optional masked fields, retains causal witnesses, rejects later source decisions and cross-symbol/grid mismatches, and is invariant to future extension. It deliberately performs no imputation. Cross-exchange inputs retain their validated Binance/Coinbase scope so composition does not depend on an unbound caller assertion. The separate research-only `missingness_aware_preprocessor_artifact_v1` learns observed-only means and scales from one exact registered training prefix, maps unavailable optional values to standardized neutral zero, and appends the ten unchanged masks for an exact 32-input signature. Its strict payload digest binds source/output schemas, split/data/source/code/runtime provenance, fit statistics, compatibility, and false downstream-authority flags; off-grid, future-dependent, corrupted, or incompatible evidence fails closed. Neither boundary is imported by production prediction or trading paths or creates a forecast or authority.
