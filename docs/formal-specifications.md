...
`A-AUTOLOOP` treats a permission-denied zero-signal PID probe as proof that the process exists, not as evidence that it is dead. Only an absent PID can downgrade persisted runner state to dead; atomic process ownership and crash-safe cycle identity remain separate open risks until their stronger contracts are implemented.
