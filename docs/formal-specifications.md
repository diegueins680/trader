...
The isolated `missingness_aware_calibrated_shallow_feature_panel_v1` composes only registered future, pre-holdout `complete_ohlcv_feature_inputs_v2` rows with optional exact-scope derivatives and Coinbase rows. It freezes twelve required 6/24-bar price fields followed by ten optional masked fields, retains causal witnesses, rejects later source decisions and cross-symbol/grid mismatches, and is invariant to future extension. It deliberately performs no imputation: a later fitted artifact must learn imputation means and scales from each training prefix only. Cross-exchange inputs retain their validated Binance/Coinbase scope so composition does not depend on an unbound caller assertion. The panel is not imported by production prediction or trading paths and creates no forecast or authority.
