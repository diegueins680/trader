...
| FEATURE-MISSINGNESS-001 | Optional predictor features can encode unavailable evidence as the same numeric zero as an observed value | HIGH | trader-firm-research | OPEN | Keep every v2 boundary isolated; build verified snapshot-complete timestamp-preserving OHLCV, external, Coinbase, and market-context artifacts plus remaining source policies and per-row market-factor construction, then version model artifacts and production builders before promotion eligibility |
