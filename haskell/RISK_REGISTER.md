...
| FEATURE-MISSINGNESS-001 | Optional predictor features can encode unavailable evidence as the same numeric zero as an observed value | HIGH | trader-firm-research | OPEN | Keep all v2 model adapters and the complete-case OHLCV boundary isolated; add verified timestamp-preserving OHLCV, external, and Coinbase source artifacts, define policies for market context and remaining direct sources, then version model artifacts and production builders before promotion eligibility |
