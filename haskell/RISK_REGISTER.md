...
| FEATURE-MISSINGNESS-001 | Optional predictor features can encode unavailable evidence as the same numeric zero as an observed value | HIGH | trader-firm-research | OPEN | Keep the derivatives v2 adapter isolated behind verified receipts; define equivalent availability policies for OHLCV, market context, Coinbase, and remaining external sources, then version artifacts and production builders before promotion eligibility |
