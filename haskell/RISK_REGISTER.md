...
| FEATURE-MISSINGNESS-001 | Optional predictor features can encode unavailable evidence as the same numeric zero as an observed value | HIGH | trader-firm-research | OPEN | Keep both v2 model adapters isolated; preserve source timestamps in a new verified external panel, define policies for OHLCV, market context, Coinbase, and direct sources, then version artifacts and production builders before promotion eligibility |
