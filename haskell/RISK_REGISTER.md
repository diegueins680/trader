...
| FEATURE-MISSINGNESS-001 | Optional predictor features can encode unavailable evidence as the same numeric zero as an observed value | HIGH | trader-firm-research | OPEN | Keep every v2 boundary and the exact shallow feature panel isolated; begin collection no earlier than 2027-01-21, independently verify and admit every frozen source, complete missing timestamp-preserving OHLCV/Coinbase source artifacts, then add training-prefix-only imputation/normalization and versioned fitted artifacts before any experiment or production eligibility |
