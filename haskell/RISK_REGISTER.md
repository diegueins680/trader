...
| FEATURE-MISSINGNESS-001 | Optional predictor features can encode unavailable evidence as the same numeric zero as an observed value | HIGH | trader-firm-research | OPEN | Keep all three v2 model adapters isolated; add verified timestamp-preserving external and Coinbase source artifacts, define policies for OHLCV, market context, and remaining direct sources, then version artifacts and production builders before promotion eligibility |
