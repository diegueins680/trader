...
normalizeMaybeText mText =
    case T.strip <$> mText of
...
uuidFromText txt =
    UUID.fromText =<< normalizeMaybeText (Just txt)

uuidToText :: UUID.UUID -> Text
uuidToText = UUID.toText

encodeJsonTextMaybe :: Maybe Aeson.Value -> Maybe Text
encodeJsonTextMaybe = fmap (TE.decodeUtf8 . BL.toStrict . Aeson.encode)

decodeJsonTextMaybe :: Maybe Text -> Maybe Aeson.Value
decodeJsonTextMaybe mText =
    case mText of
        Nothing -> Nothing
        Just txt -> Aeson.decode (BL.fromStrict (TE.encodeUtf8 txt))

data PersistedOperationRow = PersistedOperationRow
    { porId :: !Int64
    , porAtMs :: !Int64
    , porKind :: !Text
    , porSymbol :: !(Maybe Text)
    , porComboUuid :: !(Maybe UUID.UUID)
    , porOrderId :: !(Maybe Text)
    , porParams :: !(Maybe Text)
    , porArgs :: !(Maybe Text)
...
persistedOperationFromRow row =
    PersistedOperation
        ...
        , poAtMs = porAtMs row
        , poKind = porKind row
        , poSymbol = porSymbol row
        , poComboUuid = uuidToText <$> porComboUuid row
        , poOrderId = porOrderId row
        , poParams = decodeJsonTextMaybe (porParams row)
...
instance FromRow PersistedComboRow where
    fromRow =
        PersistedComboRow
            <$> field
            <*> field
...
persistedComboToValue :: PersistedComboRow -> Maybe Aeson.Value
persistedComboToValue row = do
    ...
    paramsObj <-
        case paramsVal of
            ...
            _ -> Nothing
    let metricsBase =
            case decodeJsonTextMaybe (pcrMetrics row) of
                Just (Aeson.Object obj) -> obj
                _ -> KM.empty
        metricsWithAnnualized =
            case (pcrAnnualizedReturn row, KM.lookup (AK.fromString "annualizedReturn") metricsBase >>= coerceDoubleValue) of
                (Just ann, Nothing) | not (isNaN ann || isInfinite ann) -> KM.insert (AK.fromString "annualizedReturn") (toJSON ann) metricsBase
                _ -> metricsBase
        metricsWithFinalEq =
    ...
    pure
        ( object
            ...
            , "annualizedReturn" .= pcrAnnualizedReturn row
...
opsAppend store mTenantKey kind mParams mArgs mResult mEquity mComboUuid mSymbol mOrderId = do
    ...
    let op =
            PersistedOperation
                ...
                , poKind = kind
                , poSymbol = mSymbol'
                , poComboUuid = uuidToText <$> mComboUuid'
                , poOrderId = mOrderId'
                , poParams = mParams
                , poArgs = mArgs
                , poResult = mResult
    ...
    let limitSafe = max 0 (min 5000 limit)
        ...
        baseConditions =
            catMaybes
                ...
                , fmap (\oid -> OpsCondition "order_id = ?" [toField oid]) mOrderId'
                , fmap (\fromMs -> OpsCondition "at_ms >= ?" [toField fromMs]) mFromMs
                , fmap (\toMs -> OpsCondition "at_ms <= ?" [toField toMs]) mToMs
                ]
        botCondition =
            case mBotOnly of
                Just True -> [OpsCondition "kind LIKE 'bot.%'" []]
                _ -> []
        conditions = botCondition ++ baseConditions
        whereSql =
            if null conditions
                then ""
                else " WHERE " ++ intercalate " AND " (map ocSql conditions)
        orderSql =
            if isJust sinceId
        ...
        sql =
            "SELECT id, at_ms, kind, symbol, combo_uuid, order_id, params_json::text, args_json::text, result_json::text, equity "
                ...
                <> orderSql
                <> " LIMIT ?"
        params = concatMap ocParams conditions ++ [toField limitSafe]
                            ...
                            Just st ->
                                if allowExisting
                                    then pure (Right (BotStartOutcome sym st False))
                                        ...
                                        BotRunning _ -> pure (Left "Bot is already running")
                                        BotStarting _ -> pure (Left "Bot is starting")
                            Nothing -> do
...
parseTopComboToArgs base combo = do
    ...
    let openThr = max 0 (fromMaybe (argOpenThreshold base) (tcOpenThreshold combo))
        ...
        pickMaybeMaybeDbl k current =
            fromMaybe current (getMaybeDbl k)
        ...
        pickBool k current =
            fromMaybe current (getBool k)

        pickD k current =
            fromMaybe current (readMaybeDouble k)

        pickI k current =
            fromMaybe current (readMaybeInt k)
...
withTopCombosBacktestLock ctx action =
    withMVar (tcbcLock ctx) (const action)
...
botApplyKline mOps metrics mJournal mWebhook topCombosCtx ctrl st k = do
    ...
    let nan = 0 / 0 :: Double
        ...
        holdBars =
            maybe 0 botOpenHoldingPeriods openTrade1
                ...
                Right names0 -> do
                    ...
                    if totalAfter <= maxJobs
                        then pure ()
                            ...
                            let doneSorted =
                                    sortOn
                                        (\(_path, meta) -> fromMaybe (maxBound :: Int64) (sajCreatedAtMs meta))
            ...
            case source of
                ...
                _ ->
                    case platformsRaw of
                        ...
                        _ -> Right (maybe [PlatformBinance] pure sourcePlatform)
        platformArgsResult =
    ...
    case trim <$> mDir of
        ...
        Just dir ->
            let lowered = map toLower dir
             in if null dir || lowered `elem` ["0", "false", "off", "no"]
                    then pure Nothing
                    else pure (Just dir)
    ...
    withTransaction conn $ do
        ...
        forM_ (tceCombos export) $ \combo -> do
            case uuidFromText (topComboUuid combo) of
                Nothing -> pure ()
                Just comboUuid -> do
                    let mStrategyId = HM.lookup (strategyCodeFromMethod (topComboParamString "method" combo)) strategyMap
                    ...
                    void $
                        execute
                            ...
                            , mAnnualized
                            , tcScore combo
                            , tcOpenThreshold combo
                            , tcCloseThreshold combo
                            , paramsJson
                            , metricsJson
                            , opCount
                            , now
...
persistComboParameters :: Connection -> UUID.UUID -> Maybe Int -> Aeson.Object -> IO ()
persistComboParameters conn comboUuid mStrategyId params = do
    void $ execute conn "DELETE FROM combo_parameters WHERE combo_uuid = ?" (Only comboUuid)
    forM_ (KM.toList params) $ \(k, v) ->
        void $
            execute
                conn
                "INSERT INTO combo_parameters (combo_uuid, name, value_json, strategy_id) VALUES (?, ?, ?::jsonb, ?)"
                (comboUuid, AK.toText k, encodeJsonTextMaybe (Just v), mStrategyId)
        ...
        case tradeVal of
            Aeson.Object trade -> do
                ...
                let entryEq = KM.lookup (AK.fromString "entryEquity") trade >>= AT.parseMaybe parseJSON :: Maybe Double
                    ...
                    extras =
                        catMaybes
                            [ fmap ("entryEquity" .=) entryEq
                            , fmap ("exitEquity" .=) exitEq
                            , fmap ("return" .=) retVal
                            , fmap ("holdingPeriods" .=) holding
                            , fmap ("exitReason" .=) exitReason
                            ]
            ...
            Just histDir -> do
                ...
                if not exists
                    then pure Nothing
                        ...
                        let candidates =
                                sortOn Data.Ord.Down ([histDir </> f | f <- entries, "top-combos-" `isPrefixOf` f, ".json" `isSuffixOf` f])
            ...
            case requireTenantKey "state/sync" (tenantKeyFromRequest req) of
                ...
                Right tenantKey -> do
                    ...
                    let mergedSnaps = mergeBotSnapshots (existingSnaps ++ incomingSnaps)
                        snapsWithSymbol =
                            mapMaybe
                                (\snap -> (,) <$> botSnapshotSymbol snap <*> pure snap)
                                mergedSnaps
                    ...
                    forM_ snapsWithSymbol $ uncurry (writeBotStatusSnapshotMaybe mBotStateDir tenantKey)
        ...
        Nothing ->
            respond
                ( jsonValue
                    ...
                    ( object
                        [ "enabled" .= False
                        , "hint" .= ("Set TRADER_DB_URL (or DATABASE_URL) to enable ops persistence (PostgreSQL)." :: String)
...
resolveTestnetForListenKey baseArgs = fromMaybe (argBinanceTestnet baseArgs)
    ...
    let pick :: Maybe a -> a -> a
        ...
        botSymbolsFirst =
            case apBotSymbols p of
                Nothing -> Nothing
                Just xs -> find (not . null) (map trim xs)
...
computeBinanceKeysStatusFromArgs mOps args = do
                                                ...
                                                q1 =
                                                    let mCap =
                                                            case argMaxOrderQuote args of
                                                                Just q | q > 0 -> Just q
                                                                _ -> Nothing
                                                     in maybe q0Scaled (`min` q0Scaled) mCap
  ...
  where
    ...
    fmtProbeDouble :: Double -> String
    fmtProbeDouble = trimTrailingZeros . printf "%.8f"

    fmtMaybeProbeDouble :: Maybe Double -> Maybe String
    fmtMaybeProbeDouble = fmap fmtProbeDouble

    fmtStepText :: Maybe Step -> Maybe String
    fmtStepText = fmap (T.unpack . stepText)

    fmtRawIfDifferent :: Double -> Double -> Maybe String
    fmtRawIfDifferent raw used =
        if abs (raw - used) > 1e-9 then Just (fmtProbeDouble raw) else Nothing

    formatProbeContext :: [(String, Maybe String)] -> Maybe String
    formatProbeContext fields =
        let parts = [k ++ "=" ++ v | (k, Just v) <- fields, not (null v)]
         in if null parts then Nothing else Just ("order: " ++ intercalate ", " parts)

    appendProbeContext :: Maybe String -> ApiBinanceProbe -> ApiBinanceProbe
    appendProbeContext Nothing p = p
    appendProbeContext (Just ctx) p
        | abpOk p || abpSkipped p = p
        | otherwise =
            p{abpSummary = truncateString 360 (abpSummary p ++ " (" ++ ctx ++ ")")}
...
computeCoinbaseKeysStatusFromArgs args = do
    ...
    if not (hasApiKey && hasApiSecret && hasApiPassphrase)
        then pure baseStatus
            ...
            signedProbe <- probeCoinbase "signed" (void (fetchCoinbaseAccounts env))
    ...
    if not (argConfidenceSizing args)
        then (1, Nothing)
            ...
            let hard0 = clamp01 (argLstmConfidenceHard args)
             ...
             in if hard <= 0
                    then (1, Nothing)
                    else case lstmConfidenceScore args sig of
                        Nothing -> (1, Nothing)
                        Just score ->
                            let scaleRaw
                                  | score <= soft = 0
                                    ...
                                    then (Nothing, Just "CONFORMAL_WIDTH")
                                        ...
                                        if not qWidthOk
                                            then (Nothing, Just "QUANTILE_WIDTH")
                                                ...
                                                if not (confirmConformal args thr mI dir)
...
entryScaleForSignal args market sig =
    let s0 = fromMaybe 1 (lsPositionSize sig)
        s1 = max 0 s0
...
placeOrderForSignalBot args sym sig env = placeOrderForSignalEx args sym sig env Nothing
...
placeOrderForSignalEx args sym sig env mClientOrderIdOverride enableProtectionOrders = do
  ...
  where
    ...
    minTradeQty sf price =
        let minQ = fromMaybe 0 (effectiveMinQty sf)
            stepQ = maybe 0 stepValue (effectiveStep sf)
            notionalQ =
                case sfMinNotional sf of
                    Just mn | price > 0 -> mn / price
                    _ -> 0
            minRaw = max minQ (max stepQ notionalQ)
            minAdj =
                case effectiveStep sf of
                    Just st -> quantizeUp st minRaw
                    Nothing -> minRaw
         in if minAdj > 0 then Just minAdj else Nothing

    minTradeQtyMaybe :: Maybe SymbolFilters -> Double -> Maybe Double
    minTradeQtyMaybe mSf price = mSf >>= \sf -> minTradeQty sf price

    isLongSpot :: Maybe SymbolFilters -> Double -> Double -> Bool
    isLongSpot mSf price baseBal =
        case minTradeQtyMaybe mSf price of
            Just minQ -> baseBal >= minQ
            Nothing -> baseBal > 0

    isTooSmallQtyError :: String -> Bool
    isTooSmallQtyError msg =
        msg == "Quantity rounds to 0."
            || "Quantity below minQty" `isPrefixOf` msg
    ...
    normalizeEntryQty :: SymbolFilters -> Double -> Double -> Either String (Double, Bool)
    normalizeEntryQty sf price qtyRaw =
        case normalizeQty sf price qtyRaw of
            Right q -> Right (q, False)
            Left err ->
                if not (isTooSmallQtyError err)
                    then Left err
                    else case minTradeQty sf price of
                        Nothing -> Left err
                        Just minQ ->
    ...
    placeSpotOrMargin mSf baseAsset quoteAsset dir = do
        baseBal <- fetchFreeBalance env baseAsset
        let alreadyLong = isLongSpot mSf currentPrice baseBal
                            ...
                            let quoteFromFraction =
                                    case (qtyArgBuy, quoteArgBuy, quoteFracArgBuy) of
                                        (Nothing, Nothing, Just f) ->
                                            let q0 = quoteBal * f
                                                q1 = maybe q0 (`min` q0) maxOrderQuoteArg
                                             in Just q1
                                        _ -> Nothing
                                                        ...
                                                        then
                                                            pure
                                                                baseResult
                                                                    { aorMessage =
                                                                        if bumped
                                                                            then "No order: insufficient quote balance for min order size."
                                    ...
                                    let qq = max 0 qq0
                                        minQuote =
                                            case minTradeQtyMaybe mSf currentPrice of
                                                Just minQ -> minQ * currentPrice
                                                Nothing -> 0
                                     in if qq <= 0
                                            then pure baseResult{aorMessage = "No order: quote is 0."}
                                            else
                                                if minQuote > 0 && qq < minQuote
                                                    then pure baseResult{aorMessage = "No order: quote below exchange minimums."}
                                                    else
                                                        if qq > quoteBal
                                                            then pure baseResult{aorMessage = "No order: insufficient quote balance."}
                ...
                if not alreadyLong
                    then pure baseResult{aorMessage = "No order: already flat."}
                        ...
                        case mSf of
                            ...
                            Just sf ->
                                let minQty = minTradeQty sf currentPrice
                                    baseBelowMin = maybe False (baseBal <) minQty
                                    reqBelowMin =
                                        isJust qtyArg
                                            && maybe False (\minQ -> qRaw < minQ && baseBal >= minQ) minQty
                                 in case normalizeQty sf currentPrice qRaw of
                                        Left e
                                            | baseBelowMin ->
                                                pure baseResult{aorMessage = "No order: position below exchange minimums (dust)."}
                                            | reqBelowMin ->
                                                pure baseResult{aorMessage = "No order: orderQuantity below exchange minimums."}
                                            | otherwise -> pure baseResult{aorMessage = "No order: " ++ e}
                                        Right q ->
                                            if q > baseBal
    ...
    case (ceApiKey env, ceApiSecret env, ceApiPassphrase env) of
        ...
        _ ->
            if not (argBinanceLive args)
                ...
                else case splitCoinbaseSymbol sym of
                    Left err -> noOrder ("No order: " ++ err)
...
computeTradeOnlySignal args lookback series mBinanceEnv = do
    ...
    let mPredHistory =
            if not needsHistory
                then Nothing
                else case (mKalPredHistory, mLstmPredHistory) of
...
computeBaselines periodsPerYear perSideCost prices =
    let ppy = max 1e-12 periodsPerYear
     ...
     in buyHold : maybeToList smaCrossBaseline
        ...
        stepFn (posSize, equity, eqAcc, posAcc, changes, mOpen, tradesAcc) t =
            let prev = pricesV V.! t
                ...
                (pos1, eq1, changes1, mOpen1, tradesAcc1)
                    | desired == posSize = (posSize, equity, changes, mOpen, tradesAcc)
                    | desired == 0 && posSize > 0 =
                    ...
                    | otherwise = (posSize, equity, changes, mOpen, tradesAcc)
...
resolveUrlEnv key fallback = do
    raw <- lookupEnv key
    pure $
        maybe fallback sanitizeBaseUrl (raw >>= nonEmptyTrim)
