...
optimizerArgsParser =
    OptimizerArgs
        ...
        <*> switch (long "hyperband" <> help "Run low-budget seed trials and promote survivors into the full-budget exploitation phase")
        <*> option auto (long "hyperband-seed-budget" <> value 0.35 <> metavar "FLOAT")
        <*> switch (long "surrogate-guidance" <> help "Sample score-biased interpolations from prior top-combo/JSONL rows")
        <*> option auto (long "surrogate-sample-prob" <> value 0.25 <> metavar "FLOAT")
        <*> switch (long "active-subspace" <> help "Limit surrogate interpolation to a random active subset of high-impact parameters")
        <*> option auto (long "active-subspace-param-fraction" <> value 0.45 <> metavar "FLOAT")
        <*> switch (long "map-elites" <> help "Export top combos through a diversity-preserving MAP-Elites bucket cap")
        <*> option auto (long "map-elites-per-bucket" <> value 2 <> metavar "INT")
        <*> switch (long "cma-local-search" <> help "Use CMA-inspired annealed perturbation scales in survivor exploitation")
        <*> switch (long "quality")
        ...
        <*> option auto (long "method-weight-online-nn" <> value 0.0 <> metavar "FLOAT")
        <*> option auto (long "method-weight-ta-trend" <> value 0.0 <> metavar "FLOAT")
        <*> option auto (long "method-weight-ta-reversion" <> value 0.0 <> metavar "FLOAT")
        <*> option auto (long "method-weight-ta-breakout" <> value 0.0 <> metavar "FLOAT")
        <*> option auto (long "method-weight-ta-best" <> value 0.0 <> metavar "FLOAT")
        <*> option auto (long "method-weight-ta-regime-switch" <> value 0.0 <> metavar "FLOAT")
        <*> option auto (long "method-weight-sma-cross" <> value 0.0 <> metavar "FLOAT")
        <*> option auto (long "method-weight-sma-cross-regime" <> value 0.0 <> metavar "FLOAT")
