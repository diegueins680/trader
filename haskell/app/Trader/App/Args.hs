...
import Trader.BotStartSemantics (adoptionMaxPositionSizeCap, adoptionMaxWalkForwardSharpeStd, adoptionMinEdgeFloor, adoptionMinTradeCount, adoptionMinWalkForwardSharpeMean)
...
import Trader.Method (Method (..), methodCode, methodIsTechnicalAnalysis, parseMethod, runtimeMethod)
...
data Args = Args
    ...
    , argKalmanPhysicsCandidateTrees :: Int
    , argKalmanPhysicsCandidateLearningRate :: Double
    ...
    , argAdoptionMinEdgeFloor :: Double
    , argAdoptionMinTradeCount :: Int
    , argAdoptionMinWalkForwardSharpeMean :: Double
    , argAdoptionMaxWalkForwardSharpeStd :: Double
    , argAdoptionRelaxGates :: Bool
    , argAdoptionRelaxTargetCount :: Int
...
opts = do
    ...
    argKalmanPhysicsCandidateTrees <-
        option
            auto
            ( long "kalman-physics-candidate-trees"
                <> value 0
                <> showDefault
                <> help "GBDT tree count for a fixed --method kalman_physics_error correction candidate (0 uses the built-in candidate grid)"
            )
    argKalmanPhysicsCandidateLearningRate <-
        option
            auto
            ( long "kalman-physics-candidate-learning-rate"
                <> value 0
                <> showDefault
                <> help "GBDT learning rate for a fixed --method kalman_physics_error correction candidate (0 uses the built-in candidate grid)"
            )
    ...
    argExternalData <- switch (long "external-data" <> help "Attach optional point-in-time non-exchange and exogenous feature families from configured CSV/JSON and native provider sources. Missing sources remain neutral and do not block model construction.")
    argExternalDataCsv <- many (strOption (long "external-data-csv" <> metavar "PATH" <> help "Bar-aligned alternative-data feature CSV. Recognized timestamp, optional symbol, and family columns are attached to neural and ensemble predictors. Can be repeated."))
    argExternalDataJson <- many (strOption (long "external-data-json" <> metavar "FAMILY|URL|TIME_KEY|VALUE_KEY" <> help "Generic external JSON source. FAMILY supports microstructure, options, onchain, macro, COT, news, filings, policy, fundamentals, stablecoin, institutional flows, network, developer, governance, attention, social, prediction markets, real-world, and security data. Can be repeated."))
    ...
    argTakeProfitPartial <-
        option
            ...
            ( long "take-profit-partial"
                <> value 0
                <> showDefault
                <> help "Scale out this fraction at take-profit, move the remainder stop to breakeven, and keep the runner open (0 disables; 0<frac<1)"
            )
    ...
    argAdoptionMinEdgeFloor <- option auto (long "adoption-min-edge-floor" <> value adoptionMinEdgeFloor <> showDefault <> help "Minimum stored minEdge required before live top-combo adoption")
    ...
    argAdoptionMaxWalkForwardSharpeStd <- option auto (long "adoption-max-wf-sharpe-std" <> value adoptionMaxWalkForwardSharpeStd <> showDefault <> help "Maximum stored walk-forward Sharpe standard deviation allowed before live top-combo adoption (0 disables)")
    argAdoptionRelaxGates <-
        defaultOffSwitch
            "adoption-relax-gates"
            "no-adoption-relax-gates"
            "If no combo clears the configured live-adoption gates, relax evidence thresholds to the PnL-best fully evidenced combo."
            "Keep live-adoption gates fixed."
    argAdoptionRelaxTargetCount <- option auto (long "adoption-relax-target-count" <> value 1 <> showDefault <> help "Minimum fully evidenced combos the adaptive adoption gate should admit when --adoption-relax-gates is enabled")
    argVolTarget <-
        optional
            ( option
                auto
    ...
    argBlendWeight <- option auto (long "blend-weight" <> value 0.5 <> help "Kalman weight for --method blend/conf_blend/conf_pick/conformal_clip/cost_pick/harmonic_blend/disagreement_guard/median_blend/neutral_guard/risk_parity_blend/consensus_boost/anchor_blend/tension_gate/entropy_blend/coherence_gate/divergence_gate/fractal_blend/phase_cancel/softmax_blend/smooth_softmax_blend/hedge_blend/meta_hedge_blend/net_softmax_blend/edge_blend/edge_pick/geo_blend/regime_switch (0..1)")
...
validateArgs args0 = do
    ...
    let finiteRequired =
            ...
            , ("--kalman-physics-candidate-learning-rate", argKalmanPhysicsCandidateLearningRate args)
            ...
            , ("--adoption-min-edge-floor", argAdoptionMinEdgeFloor args)
            , ("--adoption-min-wf-sharpe-mean", argAdoptionMinWalkForwardSharpeMean args)
            , ("--adoption-max-wf-sharpe-std", argAdoptionMaxWalkForwardSharpeStd args)
    ...
    ensure "--kalman-physics-candidate-trees must be >= 0" (argKalmanPhysicsCandidateTrees args >= 0)
    ensure "--kalman-physics-candidate-learning-rate must be >= 0" (argKalmanPhysicsCandidateLearningRate args >= 0)
    ...
    ensure "--method ta_trend/ta_reversion/ta_breakout/ta_best/ta_regime_switch cannot be used with --optimize-operations" $
        not (methodIsTechnicalAnalysis (argMethod args) && argOptimizeOperations args)
    ensure "--method online_nn cannot be used with --optimize-operations" $
        not (runtimeMethod (argMethod args) == MethodOnlineNeural && argOptimizeOperations args)
    ...
    ensure "--adoption-min-edge-floor must be >= 0" (argAdoptionMinEdgeFloor args >= 0)
    ensure "--adoption-min-trade-count must be >= 0" (argAdoptionMinTradeCount args >= 0)
    ensure "--adoption-max-wf-sharpe-std must be >= 0" (argAdoptionMaxWalkForwardSharpeStd args >= 0)
    ensure "--adoption-relax-target-count must be >= 1" (argAdoptionRelaxTargetCount args >= 1)
