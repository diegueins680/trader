...
opts = do
    ...
    argMethod <-
        option
            ...
            ( long "method"
                ...
                <> help "Method: 11|both=Kalman+LSTM (direction-agreement gated), blend=weighted avg, conf_blend=confidence-weighted blend, edge_blend=edge-weighted blend, geo_blend=geometric blend, router=adaptive model selection, 10|kalman=Kalman only, 01|lstm=LSTM only"
            )
    argPositioning <-
        option
    ...
    argOptimizeOperations <- switch (long "optimize-operations" <> help "Optimize method (11/10/01/blend/conf_blend/edge_blend/geo_blend/router), open-threshold, and close-threshold on a tune split (avoids lookahead on the backtest split)")
    ...
    argBlendWeight <- option auto (long "blend-weight" <> value 0.5 <> help "Kalman weight for --method blend/conf_blend/edge_blend/geo_blend (0..1)")
