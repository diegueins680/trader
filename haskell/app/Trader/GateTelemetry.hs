...
data GateTelemetry = GateTelemetry
    ...
    , gtRecentRejections    :: ![GateRejection]  -- ^ Last N rejections (bounded)
    , gtMaxRecent           :: !Int
    } deriving (Eq, Show, Generic)
...
emptyTelemetry maxRecent = GateTelemetry
    { gtTotalBars        = 0
    , gtTotalCandidates  = 0
    , gtTotalRejections  = 0
...
telemetrySummary totalBars totalCandidates totalRejections =
    let rejectionRate = if totalCandidates > 0
            then fromIntegral totalRejections / fromIntegral totalCandidates :: Double
            else 0.0
        candidateRate = if totalBars > 0
            then fromIntegral totalCandidates / fromIntegral totalBars :: Double
            else 0.0
        diagnosis = if totalCandidates == 0
            then "NO_CANDIDATES" :: Text
            else if totalRejections == totalCandidates
                then "ALL_REJECTED"
                else if rejectionRate > 0.9
