...
PRIOR_METHOD_SAMPLE_PROB="${PRIOR_METHOD_SAMPLE_PROB:-0.5}"
PRIOR_RANK_BIAS="${PRIOR_RANK_BIAS:-2.0}"
PRIOR_EDGE_WEIGHT="${PRIOR_EDGE_WEIGHT:-0.15}"
PRIOR_TOP_FRACTION="${PRIOR_TOP_FRACTION:-0.5}"
PRIOR_MIN_SAMPLES="${PRIOR_MIN_SAMPLES:-3}"
PRIOR_PERTURB_SCALE_DOUBLE="${PRIOR_PERTURB_SCALE_DOUBLE:-0.05}"
PRIOR_PERTURB_SCALE_INT="${PRIOR_PERTURB_SCALE_INT:-1}"
PRIOR_AGE_HALF_LIFE_DAYS="${PRIOR_AGE_HALF_LIFE_DAYS:-45}"
PRIOR_MISSING_AGE_MULTIPLIER="${PRIOR_MISSING_AGE_MULTIPLIER:-0.25}"
PRIOR_DIVERSITY_MAX_PER_BUCKET="${PRIOR_DIVERSITY_MAX_PER_BUCKET:-8}"
PRIOR_SEED_COUNT="${PRIOR_SEED_COUNT:-3}"

COUNT="${COUNT:-5}"
TRIALS="${TRIALS:-300}"
SURVIVOR_PARENT_ACTIVITY_FLOOR="${SURVIVOR_PARENT_ACTIVITY_FLOOR:-5}"
SURVIVOR_PARENT_ANNUALIZED_RETURN_FLOOR="${SURVIVOR_PARENT_ANNUALIZED_RETURN_FLOOR:-1.0}"
SURVIVOR_EDGE_WEIGHT="${SURVIVOR_EDGE_WEIGHT:-0.10}"
EDGE_SCORE_ANNUALIZED_RETURN_WEIGHT="${EDGE_SCORE_ANNUALIZED_RETURN_WEIGHT:-1.0}"
EDGE_SCORE_SHARPE_WEIGHT="${EDGE_SCORE_SHARPE_WEIGHT:-0.35}"
EDGE_SCORE_CALMAR_WEIGHT="${EDGE_SCORE_CALMAR_WEIGHT:-0.25}"
EDGE_SCORE_PROFIT_FACTOR_WEIGHT="${EDGE_SCORE_PROFIT_FACTOR_WEIGHT:-0.25}"
EDGE_SCORE_WALK_FORWARD_SHARPE_WEIGHT="${EDGE_SCORE_WALK_FORWARD_SHARPE_WEIGHT:-0.35}"
EDGE_SCORE_ACTIVITY_WEIGHT="${EDGE_SCORE_ACTIVITY_WEIGHT:-0.15}"
EDGE_SCORE_CALMAR_CAP="${EDGE_SCORE_CALMAR_CAP:-10}"
EDGE_SCORE_ACTIVITY_CAP="${EDGE_SCORE_ACTIVITY_CAP:-200}"
SURVIVOR_RANK_BIAS="${SURVIVOR_RANK_BIAS:-1.5}"
BARS="${BARS:-1000}"
BARS_AUTO_PROB="${BARS_AUTO_PROB:-0}"
...
NEUTRAL_RECOVERY_METHOD_WEIGHT_CROSS_SECTIONAL_MOMENTUM="${NEUTRAL_RECOVERY_METHOD_WEIGHT_CROSS_SECTIONAL_MOMENTUM:-1.0}"
...
LSTM_ADAM_BETA1="${LSTM_ADAM_BETA1:-0.9}"
LSTM_ADAM_BETA2="${LSTM_ADAM_BETA2:-0.999}"
LSTM_ADAM_EPS="${LSTM_ADAM_EPS:-1e-8}"
LSTM_ADAM_BETA1_MIN="${LSTM_ADAM_BETA1_MIN:-$LSTM_ADAM_BETA1}"
LSTM_ADAM_BETA1_MAX="${LSTM_ADAM_BETA1_MAX:-$LSTM_ADAM_BETA1}"
LSTM_ADAM_BETA2_MIN="${LSTM_ADAM_BETA2_MIN:-$LSTM_ADAM_BETA2}"
LSTM_ADAM_BETA2_MAX="${LSTM_ADAM_BETA2_MAX:-$LSTM_ADAM_BETA2}"
LSTM_ADAM_EPS_MIN="${LSTM_ADAM_EPS_MIN:-$LSTM_ADAM_EPS}"
LSTM_ADAM_EPS_MAX="${LSTM_ADAM_EPS_MAX:-$LSTM_ADAM_EPS}"
NOOPT="${NOOPT:-0}"
...
METHOD_WEIGHT_CROSS_SECTIONAL_MOMENTUM="${METHOD_WEIGHT_CROSS_SECTIONAL_MOMENTUM:-1.0}"
...
ROUTER_REGIME_MIN_BARS_MIN="${ROUTER_REGIME_MIN_BARS_MIN:-3}"
ROUTER_REGIME_MIN_BARS_MAX="${ROUTER_REGIME_MIN_BARS_MAX:-45}"
ROUTER_REGIME_MIN_FRACTION_MIN="${ROUTER_REGIME_MIN_FRACTION_MIN:-0.10}"
ROUTER_REGIME_MIN_FRACTION_MAX="${ROUTER_REGIME_MIN_FRACTION_MAX:-0.50}"
ROUTER_MIN_SCORE_MIN="${ROUTER_MIN_SCORE_MIN:-0.05}"
ROUTER_MIN_SCORE_MAX="${ROUTER_MIN_SCORE_MAX:-0.7}"
P_LONG_SHORT="${P_LONG_SHORT:-0.2}"
...
run_set() {
  ...
  for line in "${COMBOS[@]}"; do
    ...
    while :; do
      ...
      local method_weight_cross_sectional_momentum="$METHOD_WEIGHT_CROSS_SECTIONAL_MOMENTUM"
      ...
      if ((neutral_recovery_attempt > 0)); then
        ...
        method_weight_cross_sectional_momentum="$NEUTRAL_RECOVERY_METHOD_WEIGHT_CROSS_SECTIONAL_MOMENTUM"
      ...
      local cmd=("$bin"
        ...
        --bars-auto-prob "$BARS_AUTO_PROB"
        --trials "$TRIALS"
        --survivor-parent-activity-floor "$SURVIVOR_PARENT_ACTIVITY_FLOOR"
        --survivor-parent-annualized-return-floor "$SURVIVOR_PARENT_ANNUALIZED_RETURN_FLOOR"
        --survivor-edge-weight "$SURVIVOR_EDGE_WEIGHT"
        --edge-score-annualized-return-weight "$EDGE_SCORE_ANNUALIZED_RETURN_WEIGHT"
        --edge-score-sharpe-weight "$EDGE_SCORE_SHARPE_WEIGHT"
        --edge-score-calmar-weight "$EDGE_SCORE_CALMAR_WEIGHT"
        --edge-score-profit-factor-weight "$EDGE_SCORE_PROFIT_FACTOR_WEIGHT"
        --edge-score-walk-forward-sharpe-weight "$EDGE_SCORE_WALK_FORWARD_SHARPE_WEIGHT"
        --edge-score-activity-weight "$EDGE_SCORE_ACTIVITY_WEIGHT"
        --edge-score-calmar-cap "$EDGE_SCORE_CALMAR_CAP"
        --edge-score-activity-cap "$EDGE_SCORE_ACTIVITY_CAP"
        --survivor-rank-bias "$SURVIVOR_RANK_BIAS"
        --timeout-sec "$timeout_sec"
        --binary "$trader_bin"
        --lstm-adam-beta1 "$LSTM_ADAM_BETA1"
        --lstm-adam-beta2 "$LSTM_ADAM_BETA2"
        --lstm-adam-eps "$LSTM_ADAM_EPS"
        --lstm-adam-beta1-min "$LSTM_ADAM_BETA1_MIN"
        --lstm-adam-beta1-max "$LSTM_ADAM_BETA1_MAX"
        --lstm-adam-beta2-min "$LSTM_ADAM_BETA2_MIN"
        --lstm-adam-beta2-max "$LSTM_ADAM_BETA2_MAX"
        --lstm-adam-eps-min "$LSTM_ADAM_EPS_MIN"
        --lstm-adam-eps-max "$LSTM_ADAM_EPS_MAX"
        --prior-json "$PRIOR_JSON"
        --prior-sample-prob "$PRIOR_SAMPLE_PROB"
        --prior-method-sample-prob "$PRIOR_METHOD_SAMPLE_PROB"
        --prior-rank-bias "$PRIOR_RANK_BIAS"
        --prior-edge-weight "$PRIOR_EDGE_WEIGHT"
        --prior-top-fraction "$PRIOR_TOP_FRACTION"
        --prior-min-samples "$PRIOR_MIN_SAMPLES"
        --prior-perturb-scale-double "$PRIOR_PERTURB_SCALE_DOUBLE"
        --prior-perturb-scale-int "$PRIOR_PERTURB_SCALE_INT"
        --prior-age-half-life-days "$PRIOR_AGE_HALF_LIFE_DAYS"
        --prior-missing-age-multiplier "$PRIOR_MISSING_AGE_MULTIPLIER"
        --prior-diversity-max-per-bucket "$PRIOR_DIVERSITY_MAX_PER_BUCKET"
        --prior-seed-count "$PRIOR_SEED_COUNT"
        ...
        --method-weight-cross-sectional-momentum "$method_weight_cross_sectional_momentum"
        ...
        --router-regime-min-bars-min "$ROUTER_REGIME_MIN_BARS_MIN"
        --router-regime-min-bars-max "$ROUTER_REGIME_MIN_BARS_MAX"
        --router-regime-min-fraction-min "$ROUTER_REGIME_MIN_FRACTION_MIN"
        --router-regime-min-fraction-max "$ROUTER_REGIME_MIN_FRACTION_MAX"
        --router-min-score-min "$ROUTER_MIN_SCORE_MIN"
        --router-min-score-max "$ROUTER_MIN_SCORE_MAX"
        --p-long-short "$P_LONG_SHORT"
