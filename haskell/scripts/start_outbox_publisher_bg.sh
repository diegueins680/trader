...
: "${TRADER_OUTBOX_KAFKA_REST_TIMEOUT_SEC:=15}"
: "${TRADER_OUTBOX_PUBLISHED_RETENTION_MS:=604800000}"

nohup env \
  TRADER_DB_URL="${TRADER_DB_URL:-}" \
  DATABASE_URL="${DATABASE_URL:-}" \
  TRADER_OUTBOX_PUBLISHER_MODE="$TRADER_OUTBOX_PUBLISHER_MODE" \
  TRADER_OUTBOX_KAFKA_REST_URL="${TRADER_OUTBOX_KAFKA_REST_URL:-}" \
  ...
  TRADER_OUTBOX_KAFKA_REST_TIMEOUT_SEC="$TRADER_OUTBOX_KAFKA_REST_TIMEOUT_SEC" \
  TRADER_OUTBOX_PUBLISHED_RETENTION_MS="$TRADER_OUTBOX_PUBLISHED_RETENTION_MS" \
