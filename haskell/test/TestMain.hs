...
testSignalGateEntryFeeBuffer = do
    ...
    assert "headroom-only helper remains the zero-fee specialization" (signalEntryHeadroomOk 0.01 (Just 0.015) == signalEntryFeeBufferOk 0.01 0 (Just 0.015))
...
testSignalGateEntryFeeBufferMonotoneFees = do
    ...
    let blockedLadder = map (\fee -> signalEntryFeeBufferOk 0.01 fee (Just 0.0165)) [0.002, 0.0035, 0.004]
    ...
    assert "once blocked at a given fee floor, larger fee floors stay blocked" (blockedLadder == [False, False, False])
    assertMonotoneNonIncreasing "blocked fee-aware states stay blocked as fee floors rise" blockedLadder
