...
- **Market-context source v1:** each new bundle contains exact public response bytes, a `complete_unverified` collection status, and a `binance_usdm_market_context_source_manifest_v1` published only after full eligible-population coverage. The offline verifier independently derives the v2 panel and a separate non-authorizing receipt. No real bundle exists as of 2026-09-08; synthetic fixtures only exercise the contract.
