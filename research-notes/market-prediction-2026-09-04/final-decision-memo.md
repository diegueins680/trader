...
| Missingness-aware calibrated shallow model | Continue research / blocked | Implement and verify raw acquisition for the new prospective market-context panel, complete remaining timestamp-preserving source and fitted-artifact boundaries, then wait for the registered future window |
...
- `FEATURE-MISSINGNESS-001` is open: an unavailable optional feature may be encoded as the same zero as an observed value, and legacy market context can apply mixed-vintage or cutoff-time universe evidence across earlier rows. The additive core, complete-case OHLCV boundary, point-in-time liquidity selector, prospective market-context panel decoder, raw market-factor adapter, and derivatives, external-family, and Coinbase cross-exchange v2 adapters now represent required/masked causal evidence in isolation. The panel decoder binds declared completeness and a source-manifest digest but cannot verify its raw bytes; the raw factor selects membership per row and has complete-input parity with the peer-basket `weightedMarketLag` invocation without optional Coinbase augmentation. Affected learned candidates remain ineligible until raw source completeness, fitted-model artifacts, builders, and every remaining boundary are versioned and verified.
