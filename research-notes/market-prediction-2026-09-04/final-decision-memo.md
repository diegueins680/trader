...
| Missingness-aware calibrated shallow model | Continue research / blocked | Complete timestamp-preserving source artifacts and versioned production/artifact boundaries for the isolated v2 complete-case OHLCV, derivatives, external-family, cross-exchange, and point-in-time universe contracts, then wait for the registered future window |
...
- `FEATURE-MISSINGNESS-001` is open: an unavailable optional feature may be encoded as the same zero as an observed value, and legacy market context can apply mixed-vintage or cutoff-time universe evidence across earlier rows. The additive core, complete-case OHLCV boundary, point-in-time liquidity snapshot selector, and derivatives, external-family, and Coinbase cross-exchange v2 adapters now represent required/masked causal evidence in isolation, but affected learned candidates remain ineligible until every source artifact, builder, per-row context rule, and model boundary is versioned and verified.
