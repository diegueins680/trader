...
The only production-path code change in the original review was semantic/provenance plumbing: stable legacy sequence-predictor identifiers gained truthful versioned implementation identities and compatible aliases. The later missingness-aware feature panel is isolated research code and is not registered or loaded by any production path. Neither change alters a forecast, combo, saved configuration, fitted artifact, risk threshold, deployment mode, or order path.
...
7. OFI lacks event-level L2 history and fill replay; the calibrated shallow candidate now has an isolated exact masked feature panel but no admitted prospective sources or fitted artifact; HAR-RV needs genuinely new prospective evidence. These are research blockers, not gates to waive.
...
| Missingness-aware calibrated shallow model | Continue research / blocked | Preserve the frozen `missingness_aware_calibrated_shallow_feature_panel_v1`; run bounded public collection no earlier than 2027-01-21, independently admit every frozen source, add the fold-local fitted-artifact boundary, and do not inspect the final holdout before its one-shot gate |
