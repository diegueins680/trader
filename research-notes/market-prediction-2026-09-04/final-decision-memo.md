...
| Missingness-aware calibrated shallow model | Continue research / blocked | Complete timestamp-preserving source artifacts and versioned production/artifact boundaries for the isolated v2 complete-case OHLCV, derivatives, external-family, cross-exchange, point-in-time universe, and per-row raw market-factor contracts, then wait for the registered future window |
...
- `FEATURE-MISSINGNESS-001` is open: an unavailable optional feature may be encoded as the same zero as an observed value, and legacy market context can apply mixed-vintage or cutoff-time universe evidence across earlier rows. The additive core, complete-case OHLCV boundary, point-in-time liquidity selector and raw market-factor adapter, and derivatives, external-family, and Coinbase cross-exchange v2 adapters now represent required/masked causal evidence in isolation. The raw factor selects membership per row and has complete-input parity with the peer-basket `weightedMarketLag` invocation without optional Coinbase augmentation, but affected learned candidates remain ineligible until source completeness, peer-return provenance, fitted-model artifacts, builders, and every remaining boundary are versioned and verified.
