...
The only production-path code change in the original review was semantic/provenance plumbing: stable legacy sequence-predictor identifiers gained truthful versioned implementation identities and compatible aliases. The later missingness-aware feature panel and preprocessor artifact are isolated research code and are not registered or loaded by any production path. Neither boundary alters a forecast, combo, saved configuration, fitted return-model artifact, risk threshold, deployment mode, or order path.
...
7. OFI lacks event-level L2 history and fill replay; the calibrated shallow candidate now has an isolated exact masked feature panel and strict preprocessor-artifact builder but no admitted prospective sources or fitted return-model artifact; HAR-RV needs genuinely new prospective evidence. These are research blockers, not gates to waive.
...
| Missingness-aware calibrated shallow model | Continue research / blocked | Preserve the frozen feature-panel and preprocessor contracts; run bounded public collection no earlier than 2027-01-21, independently admit every frozen source, fit the preprocessor and return model within each training prefix, and do not inspect the final holdout before its one-shot gate |
...
- `FEATURE-MISSINGNESS-001` is open: legacy production predictors can still encode an unavailable optional feature as the same zero as an observed value, and legacy market context can apply mixed-vintage or cutoff-time universe evidence across earlier rows. The additive core, complete-case OHLCV boundary, point-in-time liquidity selector, prospective market-context collector/panel decoder/offline raw-source verifier, bundle verifier, frozen-archive receipt replay, raw market-factor adapter, and derivatives, external-family, and Coinbase cross-exchange v2 adapters represent required/masked causal evidence in isolation. The `missingness_aware_calibrated_shallow_feature_panel_v1` composes only the registered complete-OHLCV, derivatives, and same-symbol spot-context families on an exact scope/grid, freezes twelve required price features plus ten optional masked fields, rejects pre-registration and holdout rows, and performs no imputation. Its isolated preprocessor artifact now freezes training-prefix-only observed statistics, neutral mean imputation, masks, provenance, and strict compatibility without fitting a return model. The collector has not run and cannot start before the registered 2027-01-21 boundary; future source evidence still needs independent admission, and prospective preprocessor plus fitted-model artifacts do not yet exist. Affected learned candidates remain ineligible until sufficient verified prospective history, every fitted artifact/builder boundary, and all evaluation gates are versioned and reviewed.
