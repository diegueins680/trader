...
| Missingness-aware calibrated shallow model | Continue research / blocked | Complete timestamp-preserving source artifacts and versioned production/artifact boundaries for the isolated v2 complete-case OHLCV, derivatives, external-family, and cross-exchange contracts, then wait for the registered future window |
...
- `FEATURE-MISSINGNESS-001` is open: an unavailable optional feature may be encoded as the same zero as an observed value. The additive core, complete-case OHLCV boundary, and derivatives, external-family, and Coinbase cross-exchange v2 adapters now represent required/masked causal evidence in isolation, but affected learned candidates remain ineligible until every source artifact, builder, and model boundary is versioned and verified.
