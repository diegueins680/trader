...
The strict JSON envelope records schema and compatibility version 1, the source feature schema and signatures, exact output signature, registration and academic origins, code commit, training-data/panel/source/split SHA-256 identities, symbol, interval, horizon, train/validation/holdout boundaries, purge and embargo, the latest training decision, fit and creation times, seed, runtime versions, cost-model identity, row and observation counts, and fitted means/scales. The fit time must be no earlier than both the final training bar end and the recorded latest decision, while remaining before validation. A SHA-256 digest covers the complete payload. Unknown fields, corruption, incompatible schemas, unsupported registered values, and any true research-admission, experiment, holdout, model, promotion, deployment, order, or live-trading authority flag are rejected.
