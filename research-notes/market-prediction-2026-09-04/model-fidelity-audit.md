...
| Market-context model | Faithful simple linear factor with material data caveats | Volume-weighted market lag and OLS target relation; optional Coinbase context | One cutoff-time universe/weight vector is applied across the history; current-volume fallback can leak survivor/universe information; legacy PIT rows do not separate event from availability or prove snapshot completeness |
...
- `Features.barAt` also synthesizes missing open/high/low from closes and missing volume as `1`. Live venue loaders now provide validated source OHLCV, but close-only/offline callers can still produce apparently complete kline features. The isolated `complete_ohlcv_feature_inputs_v2` boundary added on 2026-09-08 instead requires every OHLCV field plus causal first-seen timing and reproduces legacy formulas only after complete-case admission.
- `PointInTimeUniverse` chooses the latest row independently per symbol at or before one cutoff, imposes no age bound, and treats the row timestamp as its availability. `MarketContext` then applies that single selected membership and quote-volume weight vector across the earlier price history; unless `TRADER_MARKET_CONTEXT_REQUIRE_PIT_UNIVERSE` is true, it may instead rank the present exchange snapshot. The isolated `point_in_time_liquidity_universe_v2` contract added on 2026-09-08 requires one coherent snapshot, separate first-seen timing, bounded event age, explicit contemporaneous eligibility, deterministic ranking, and revision-safe decisions. The separate `point_in_time_market_context_factor_v2` adapter consumes a new selection for each bar, excludes the target, renormalizes peer weights, and reproduces only the peer-basket portion of the raw legacy weighted-return formula—with no optional Coinbase augmentation—using causal witnesses and an explicit mask. Its abstract factor retains the validated target, quote, selected peer identities, bar, interval, and row behind a private constructor, preventing later scope relabeling. The `binance_usdm_market_context_panel_v2` decoder now binds a derived prospective population and optional peers to complete counts, rolling-window clocks, and a frozen manifest digest before projecting through that selector. The research-only `point_in_time_market_context_linear_ols_v1` component now adds a distinct fold-local OLS artifact that consumes this scoped factor directly, admits causal post-close universe witnesses, and derives scope-bound labels from canonical complete target bars, with exact grids, purge/embargo, provenance, strict compatibility, corruption detection, and missing-factor exclusion. It is deliberately not imported into the legacy model or any predictor, and is not a complete candidate artifact, ranker, data-admission decision, experiment runner, or production builder.
...
The separate offline `binance_usdm_market_context_source_manifest_v1` verifier closes the derived panel's raw-byte reconstruction gap for a frozen bundle: it independently hashes and decodes the registered responses, recomputes eligibility and close-to-close peer returns, and reproduces the exact Haskell-readable panel plus a non-authorizing receipt. A second offline wrapper closes the collector-to-source trust gap by accepting only exact `complete_unverified` status, verifying status/manifest clocks and inventory plus the historical Git-bound collector, verifier, and public-source/license blobs, and binding the independent source receipt into its own non-authorizing receipt. A third read-only verifier closes the persisted-receipt replay gap by requiring the historical bundle-verifier bytes, hashing an exact no-extra-file archive before and after reconstruction, and matching the recomputed receipt. A bounded public-only collector preserves the required prospective bytes, enforces the preregistered data window, and has not been run. The fitted-component contract now exists, but sufficient prospective history, explicit data admission, complete candidate-level artifact provenance, remaining timestamp-preserving sources, and production builders remain material gaps.
