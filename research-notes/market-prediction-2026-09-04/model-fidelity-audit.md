...
- `PointInTimeUniverse` chooses the latest row independently per symbol at or before one cutoff, imposes no age bound, and treats the row timestamp as its availability. `MarketContext` then applies that single selected membership and quote-volume weight vector across the earlier price history; unless `TRADER_MARKET_CONTEXT_REQUIRE_PIT_UNIVERSE` is true, it may instead rank the present exchange snapshot. The isolated `point_in_time_liquidity_universe_v2` contract added on 2026-09-08 requires one coherent snapshot, separate first-seen timing, bounded event age, explicit contemporaneous eligibility, deterministic ranking, and revision-safe decisions. It remains incomplete as a source solution because a pure constructor cannot prove full venue-population coverage.
