...
def _validate_registry_policy(policy: Mapping[str, object]) -> None:
    if C.SHARED_REPOSITORY_RESOLUTION_ERROR is not None:
        raise ValueError("official shared holdout registry cannot be resolved")
    registered_registry = (
        C.SHARED_REPOSITORY_ROOT / str(policy["registryDirectory"])
    ).resolve()
    if (
    ...
    if not TEST_ONLY_ALLOW_REGISTRY_OVERRIDE:
        C._assert_shared_registry_reconciled(
            C.REPOSITORY_ROOT,
